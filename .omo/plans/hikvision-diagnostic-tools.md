...
- [ ] 8. `tools/hikvision/isapi_client.py` — Add `set_motion_config()` (TDD)

  **What to do**:
  - **RED**: Write unit tests: `test_set_motion_config_full_update`, `test_set_motion_config_partial_update`, `test_set_motion_config_failure`
  - **GREEN**: Add `set_motion_config(self, enabled: bool | None = None, sensitivity: int | None = None) -> bool`
  - **Read-modify-write pattern**: (1) Call `self.get_motion_config()` to get current state, (2) If None, use sensible defaults (enabled=True, sensitivity=70, regionType="grid", rowGranularity="18x18", columnGranularity="22x22", gridMap from known default), (3) Modify only the fields specified, (4) Construct complete XML with `ElementTree` preserving all fields, (5) PUT with `Content-Type: application/xml`
  - XML must match the exact structure: `<MotionDetection xmlns="{ISAPI_NS}">` with `<enabled>`, `<regionType>`, `<Grid>`, `<MotionDetectionLayout>` sub-elements
  - Serialize with `ET.tostring(root, encoding="unicode", xml_declaration=True)`
  - Return `True` on success, `False` on `except Exception`
  - **REFACTOR**: Verify all 3 tests pass

  **Must NOT do**:
  - Do not send partial XML — the doorbell REQUIRES complete XML on PUT (missing fields → `badParameters` error)
  - Do not construct XML by string concatenation — use `ElementTree` for namespace-correct generation

  **Recommended Agent Profile**:
  - **Category**: `unspecified-high` — ISAPI XML construction, read-modify-write pattern
  - **Skills**: None needed

  **Parallelization**:
  - **Can Run In Parallel**: NO — depends on Task 3
  - **Parallel Group**: Wave 1
  - **Blocks**: Task 13 (set_motion_detection wrapper)
  - **Blocked By**: Task 3 (`get_motion_config`)

  **References**:
  - `tools/hikvision/isapi_client.py:24` — `ISAPI_NS` constant for namespace
  - `tools/hikvision/isapi_client.py:100-117` — `open_door()` — PUT with XML body and `Content-Type: application/xml` header
  - `tests/unit/test_iot_hikvision.py:128-146` — PUT mock pattern (`MockSession.return_value.put.return_value`)

  **Acceptance Criteria**:
  - [ ] `pytest tests/unit/test_iot_hikvision.py::TestHikvisionISAPIClient::test_set_motion_config_full_update -v` → PASS
  - [ ] `pytest tests/unit/test_iot_hikvision.py::TestHikvisionISAPIClient::test_set_motion_config_partial_update -v` → PASS
  - [ ] `pytest tests/unit/test_iot_hikvision.py::TestHikvisionISAPIClient::test_set_motion_config_failure -v` → PASS
  - [ ] PUT body contains `<enabled>false</enabled>` AND all original fields preserved

  **QA Scenarios**:
  ```
  Scenario: set_motion_config full update (enabled=True, sensitivity=60)
    Tool: Bash (pytest)
    Steps:
      1. pytest tests/unit/test_iot_hikvision.py::TestHikvisionISAPIClient::test_set_motion_config_full_update -v --tb=short
    Expected Result: PASS, returns True, PUT body contains enabled=true AND sensitivity=60 AND all original fields (regionType, Grid, gridMap)
    Evidence: .omo/evidence/task-8-set-motion-full.txt

  Scenario: set_motion_config partial update (only enabled, sensitivity unchanged)
    Tool: Bash (pytest)
    Steps:
      1. pytest tests/unit/test_iot_hikvision.py::TestHikvisionISAPIClient::test_set_motion_config_partial_update -v --tb=short
    Expected Result: PASS, PUT body has enabled=false but sensitivity still 70 (unchanged)
    Evidence: .omo/evidence/task-8-set-motion-partial.txt

  Scenario: set_motion_config returns False on ISAPI 400
    Tool: Bash (pytest)
    Steps:
      1. pytest tests/unit/test_iot_hikvision.py::TestHikvisionISAPIClient::test_set_motion_config_failure -v --tb=short
    Expected Result: PASS, returns False
    Evidence: .omo/evidence/task-8-set-motion-failure.txt
  ```

  **Commit**: YES
  - Message: `feat(hikvision): add set_motion_config ISAPI client method`
  - Files: `tools/hikvision/isapi_client.py`, `tests/unit/test_iot_hikvision.py`

- [ ] 9. `tools/iot_hikvision.py` — Add `_hikvision_get_motion_config()` wrapper + registration (TDD)

  **What to do**:
  - **RED**: Write unit tests in `TestHikvisionTools`: `test_get_motion_config_success`, `test_get_motion_config_error`
  - **GREEN 1**: Write internal function `_hikvision_get_motion_config() -> str` — follows ISAPI pattern B: `try → create_isapi_client() → client.get_motion_config() → if config: _success_response(config) else ISAPI_ERROR → except ValueError: MISSING_CREDENTIALS → except Exception: INTERNAL_ERROR`
  - **GREEN 2**: Add `@mcp.tool()` registration `hikvision_get_motion_config() -> str` inside `register_hikvision_tools()` — follows READ pattern: `start_tool_context() → increment_tool_count("hikvision_get_motion_config") → return _hikvision_get_motion_config()`
  - Docstring must include `@since v1.5.0`
  - **REFACTOR**: Verify tests pass

  **Must NOT do**:
  - Do not add `check_write_enabled()` — this is a READ tool
  - Do not add `ValidationError` handler — no write guard

  **Recommended Agent Profile**:
  - **Category**: `unspecified-high` — tool wrapper + registration, follows exact existing pattern
  - **Skills**: None needed
  ...
  **Parallelization**:
  ...
  - **Blocked By**: Task 3 (`get_motion_config` ISAPI method)

  **References**:
  - `tools/iot_hikvision.py:122-136` — `_hikvision_device_info()` — ISAPI read pattern (try/except ValueError/Exception)
  - `tools/iot_hikvision.py:312-334` — `hikvision_device_info` registration — READ tool registration pattern

  **Acceptance Criteria**:
  - [ ] `pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_motion_config_success -v` → PASS
  - [ ] `pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_motion_config_error -v` → PASS
  - [ ] `_hikvision_get_motion_config` in `__all__` (already added in Task 2)

  **QA Scenarios**:
  ```
  Scenario: get_motion_config returns success with enabled/sensitivity/grid_map
    Tool: Bash (pytest)
    Steps:
      1. pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_motion_config_success -v --tb=short
    Expected Result: PASS, response contains "success": true, "enabled", "sensitivity", "grid_map" keys
    Evidence: .omo/evidence/task-9-wrapper-motion-config-success.txt

  Scenario: get_motion_config returns MISSING_CREDENTIALS on ValueError
    Tool: Bash (pytest)
    Steps:
      1. pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_motion_config_error -v --tb=short
    Expected Result: PASS, response has "success": false, error code "MISSING_CREDENTIALS"
    Evidence: .omo/evidence/task-9-wrapper-motion-config-error.txt
  ```

  **Commit**: YES (groups with Tasks 10-12)
  - Message: `feat(hikvision): register get_motion_config/get_event_config/get_alarm_server/snapshot_to_file tools`
  - Files: `tools/iot_hikvision.py`

- [ ] 10. `tools/iot_hikvision.py` — Add `_hikvision_get_event_config()` wrapper + registration (TDD)

  **What to do**:
  - **RED**: Write unit tests `test_get_event_config_success`, `test_get_event_config_error`
  - **GREEN**: Write `_hikvision_get_event_config() -> str` — ISAPI read pattern, calls `client.get_event_triggers()`, wraps in `_success_response({"triggers": triggers, "count": len(triggers)})`
  - Add `@mcp.tool()` registration `hikvision_get_event_config()` — READ pattern
  - Docstring must include `@since v1.5.0`

  **Must NOT do**:
  - Do not add write guard
  ...
  **Parallelization**:
  ...
  - **Parallel Group**: Wave 2
  - **Blocks**: Task 16
  - **Blocked By**: Task 4 (`get_event_triggers` ISAPI method)

  **Acceptance Criteria**:
  - [ ] `pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_event_config_success -v` → PASS
  - [ ] Response includes `"triggers"` list and `"count"` field

  **QA Scenarios**:
  ```
  Scenario: get_event_config returns trigger list with count
    Tool: Bash (pytest)
    Steps:
      1. pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_event_config_success -v --tb=short
    Expected Result: PASS, data.triggers is list, data.count matches len(triggers)
    Evidence: .omo/evidence/task-10-event-config-success.txt
  ```

  **Commit**: YES (groups with 9, 11, 12)

- [ ] 11. `tools/iot_hikvision.py` — Add `_hikvision_get_alarm_server()` wrapper + registration (TDD)

  **What to do**:
  - **RED**: Write unit tests `test_get_alarm_server_success`, `test_get_alarm_server_error`
  - **GREEN**: Write `_hikvision_get_alarm_server() -> str` — ISAPI read pattern, calls `client.get_alarm_server()`, wraps in `_success_response({"alarm_server": server})`
  - Add `@mcp.tool()` registration `hikvision_get_alarm_server()` — READ pattern
  - Docstring must include `@since v1.5.0`
  ...
  **Parallelization**:
  ...
  - **Parallel Group**: Wave 2
  - **Blocks**: Task 16
  - **Blocked By**: Task 5 (`get_alarm_server` ISAPI method)

  **Acceptance Criteria**:
  - [ ] `pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_alarm_server_success -v` → PASS
  - [ ] Response includes `"alarm_server"` dict with url, ip, port fields

  **QA Scenarios**:
  ```
  Scenario: get_alarm_server returns alarm server config
    Tool: Bash (pytest)
    Steps:
      1. pytest tests/unit/test_iot_hikvision.py::TestHikvisionTools::test_get_alarm_server_success -v --tb=short
    Expected Result: PASS, data.alarm_server has url, ip, port, protocol, auth_method
    Evidence: .omo/evidence/task-11-alarm-server-success.txt
  ```

  **Commit**: YES (groups with 9-10, 12)

- [ ] 12. `tools/iot_hikvision.py` — Add `_hikvision_snapshot_to_file()` wrapper + registration (TDD)
