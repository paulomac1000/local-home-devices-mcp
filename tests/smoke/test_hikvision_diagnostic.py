...
class TestHikvisionDiagnosticSmoke:
    ...
    def test_get_motion_config_returns_success(self):
        resp = requests.post(
            f"{REST_API_URL}/api/tools/hikvision_get_motion_config",
            json={},
            timeout=10,
        )
        data = resp.json()
    ...
    def test_get_event_config_returns_success(self):
        resp = requests.post(
            f"{REST_API_URL}/api/tools/hikvision_get_event_config",
            json={},
            timeout=10,
        )
        data = resp.json()
    ...
    def test_get_alarm_server_returns_success(self):
        resp = requests.post(
            f"{REST_API_URL}/api/tools/hikvision_get_alarm_server",
            json={},
            timeout=10,
        )
        data = resp.json()
    ...
    def test_snapshot_to_file_returns_validation_error(self):
        ...
        resp = requests.post(
            f"{REST_API_URL}/api/tools/hikvision_snapshot_to_file",
            json={},
            timeout=10,
        )
        data = resp.json()
    ...
    def test_set_motion_detection_returns_not_write_disabled(self):
        ...
        assert "write" in data["result"]["error"]["message"].lower()

    def test_isapi_health_returns_overall(self):
        resp = requests.post(
            f"{REST_API_URL}/api/tools/hikvision_isapi_health",
            json={"since": "4h"},
            timeout=10,
        )
        data = resp.json()
        assert data["result"]["success"] is True
        assert "overall" in data["result"]["data"]

    def test_pipeline_diagnose_returns_layers(self):
        resp = requests.post(
            f"{REST_API_URL}/api/tools/hikvision_pipeline_diagnose",
            json={},
            timeout=10,
        )
        data = resp.json()
