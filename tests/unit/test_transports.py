...
NOTE: We mock the ENTIRE 'fastmcp' module before importing 'server' because the
installed fastmcp 2.0.0 package has broken transitive dependencies (missing
fastmcp.mcp_config, fastmcp.client.auth, etc.). server.py only uses FastMCP
for the module-level 'mcp' instance; our route tests use create_rest_app()
which is self-contained via Starlette.
"""
...
# server.py does "from fastmcp import FastMCP" at module level, and the
# installed fastmcp 2.0.0 is missing several transitive submodules
# (mcp_config, client.auth, etc.).  A MagicMock satisfies everything
# server.py needs from the fastmcp package.
# ---------------------------------------------------------------------------
_fastmcp_mock = MagicMock()
_FastMCP_cls = MagicMock()
...
class TestTransportRoutes:
    ...
    def test_mcp_delete_with_session_terminates(self):
        ...
        client = TestClient(self._create_app())
        resp = client.delete("/mcp", headers={"mcp-session-id": session_id, "Origin": "http://localhost:9102"})
        assert resp.status_code == 200
        assert _validate_session(session_id) is False
